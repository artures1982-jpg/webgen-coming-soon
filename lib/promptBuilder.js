...
function buildUserPrompt(styleConfig, firma) {
  ...
  const schema = JSON.stringify({
    ...
    description: nazwaStrony + ' — ' + branza + ' w mieście ' + lok,
...
${obszarList.length ? `Obszary działania:  ${obszarList.join(', ')}` : `Obszary działania: głównie w mieście ${lok} i okolicy — wygeneruj sekcję z opisem zasięgu`}
...
<meta name="description" content="${nazwa} — profesjonalne usługi ${branza} w mieście ${lok}. Zadzwoń: ${tel}">
...
<meta property="og:description" content="${nazwa} — profesjonalne usługi ${branza} w mieście ${lok}. ${firma.realizacje ? firma.realizacje + ' realizacji. ' : ''}Zadzwoń: ${tel}">
...
${faqList.map((f, i) => `  ${i + 1}. P: ${f.q}
     O: ${f.a}`).join('\n')}` : `FAQ: wygeneruj 4-5 pytań i odpowiedzi specyficznych dla ${branza} w mieście ${lok}`}
KONTAKT — adres, telefon ${tel}, email ${email}, godziny + mapa dojazdu: prawdziwy, działający embed
Google Maps bez klucza API, format <iframe src="https://www.google.com/maps?q=ADRES,+MIASTO&output=embed"
loading="lazy" referrerpolicy="no-referrer-when-downgrade" style="border:0;width:100%;height:320px">
</iframe> — podstaw realny adres/lokalizację (${firma.adres ? firma.adres + ', ' : ''}${lok}) do q=,
NIE zostawiaj placeholdera/prostokąta zastępczego
...
- Treści 100% realne, specyficzne dla ${branza} w mieście ${lok} — ZERO lorem ipsum
- NIGDY nie odmieniaj nazwy miasta przez przyimek bezpośrednio (np. "w ${lok}" jest błędne
  gramatycznie dla większości miast, jak "w Kraków" zamiast "w Krakowie") — używaj bezpiecznych
  konstrukcji działających dla każdej nazwy w mianowniku: "w mieście X", "na terenie miasta X",
  "poza miastem X", albo myślnika/dwukropka zamiast przyimka ("Usługi — X")
...
zmieni.
UWAGA GRAMATYCZNA: {{MIASTO}} zostanie podstawione DOWOLNĄ nazwą polskiego miasta w mianowniku —
nigdy nie wstawiaj go bezpośrednio po przyimku wymagającym odmiany ("w {{MIASTO}}", "z {{MIASTO}}",
"poza {{MIASTO}}" — to błędne dla większości miast, np. "w Kraków" zamiast "w Krakowie"). Zamiast
tego pisz "w mieście {{MIASTO}}", "na terenie miasta {{MIASTO}}", "poza miastem {{MIASTO}}", albo
używaj myślnika/dwukropka zamiast przyimka. {{MIASTO}} użyte samodzielnie bez przyimka (adres,
tytuł, tag lokalizacji, mianownik w zdaniu) jest zawsze bezpieczne.`;
