// Port 1:1 z api/generate.js — ścieżka zapasowa generatora (gdy klient nie wybrał
// jeszcze szablonu z Galerii Startowej). Logika/prompt bez zmian; runtime celowo NIE
// ustawiony na Edge (oryginał był Edge) — zgodnie z aktualną wytyczną Vercela w tym
// środowisku Node + Fluid Compute jest tak samo szybki i ma pełne API, bez ograniczeń
// Edge. To jedyna świadoma zmiana względem oryginału w tym porcie.
type Firma = {
  style?: string;
  nazwa?: string;
  dzielnica?: string;
...
const STYLES: Record<string, { name: string; system: string; visual: string }> = {
  classic: {
    name: "Klasyczny",
    system:
      "Jesteś ekspertem web designu tworzącym profesjonalne strony dla polskich firm lokalnych.\n" +
    ...
    visual:
      "STYL: KLASYCZNY — ciepły, lokalny, profesjonalny, godny zaufania.\n" +
  ...
  modern: {
    name: "Nowoczesny",
    system:
      "Jesteś ekspertem web designu tworzącym premium strony dla polskich firm lokalnych.\n" +
      "Tworzysz WYŁĄCZNIE kompletny, gotowy do użycia kod HTML z wbudowanym CSS.\n" +
      "Nie dodajesz żadnych komentarzy, wyjaśnień ani markdown — tylko czysty HTML.",
    visual:
      "STYL: NOWOCZESNY — dark mode, tech, premium, odważny.\n" +
  ...
  elegant: {
    name: "Elegancki",
    system:
      "Jesteś ekspertem web designu tworzącym luksusowe strony dla polskich firm lokalnych.\n" +
      "Tworzysz WYŁĄCZNIE kompletny, gotowy do użycia kod HTML z wbudowanym CSS.\n" +
      "Nie dodajesz żadnych komentarzy, wyjaśnień ani markdown — tylko czysty HTML.",
    visual:
      "STYL: ELEGANCKI — luksusowy, minimalistyczny, premium, atelier.\n" +
...
// Ochrona kosztowa — endpoint jest wywoływalny bezpośrednio, allowlist Origin/Referer +
// licznik w pamięci modułu (nie twarda gwarancja, tylko podniesienie poprzeczki).
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!firma) return Response.json({ error: "Brak danych firmy" }, { status: 400, headers: corsHeaders });

    const ANTHROPIC_KEY = process.env.ANTHROPIC_API_KEY;
    if (!ANTHROPIC_KEY) {
      return Response.json({ error: "Brak ANTHROPIC_API_KEY w środowisku" }, { status: 500, headers: corsHeaders });
    }
...
4. USŁUGI — ${
      Array.isArray((firma as { uslugi_lista?: string[] }).uslugi_lista) &&
      (firma as { uslugi_lista?: string[] }).uslugi_lista!.length > 0
        ? "karty usług DOKŁADNIE dla tych usług (po jednej karcie na usługę): " +
          (firma as { uslugi_lista?: string[] }).uslugi_lista!.join(", ")
...
ODPOWIEDZ WYŁĄCZNIE KODEM HTML. Pierwsza linia kodu: <!DOCTYPE html>  Ostatnia linia: </html>`;
    ...
    if (!r.ok) {
      ...
      return Response.json({ error: `Claude API ${r.status}: ${errMsg.slice(0, 300)}` }, { status: 500, headers: corsHeaders });
    }
    ...
    try {
      ...
      return Response.json({ error: "Nieprawidlowa odpowiedz API: " + rawText.slice(0, 100) }, { status: 500, headers: corsHeaders });
    }
